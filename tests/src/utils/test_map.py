...
class TestMap:
    ...
    def test_get_deployment_zone(self, mock_random_choice, sample_zones_for_test_map):
        ...
        deployment_zone = Map.Zone.get_deployment_zone(sample_zones_for_test_map)
    ...
    def test_get_closest_zone(self, sample_zones_for_test_map):
        """
        Test that get_closest_zone returns the zone 
        with the shortest distance to the given position.
        """
        position = (0.5, 0.5)  # Inside zone 1
        closest_zone = Map.Position.get_closest_zone(sample_zones_for_test_map, position)
    ...
    def test_get_speed_limit_inside_zone(
            self, sample_zones_for_test_map, sample_zone_types_for_test_map):
        """
        Test that get_speed_limit returns the 
        correct speed limit for a position inside a zone.
        """
        position = (0.5, 0.5)  # Inside zone 1 (parking)
        speed_limit = Map.Zone.get_speed_limit(
            sample_zones_for_test_map, sample_zone_types_for_test_map, position)
    ...
    def test_get_speed_limit_outside_zone(
            self, sample_zones_for_test_map, sample_zone_types_for_test_map):
        """
        Test that get_speed_limit returns the speed limit 
        of the closest zone when the position is outside all zones.
        """
        position = (10.0, 10.0)  # Outside all zones
        # The closest zone in sample_zones is zone 4 with speed_limit 5
        expected_speed_limit = \
            sample_zone_types_for_test_map["parking"]["speed_limit"]  # 5
        speed_limit = \
            Map.Zone.get_speed_limit(
                sample_zones_for_test_map,
                sample_zone_types_for_test_map, position)
    ...
    def test_is_charging_zone_false(self, sample_zones_for_test_map):
        """
        Test that is_charging_zone returns False for positions not inside charging zones.
        """
        position = (0.5, 0.5)  # Inside zone 1 (parking)
        assert Map.Zone.is_charging_zone(sample_zones_for_test_map, position) is False
