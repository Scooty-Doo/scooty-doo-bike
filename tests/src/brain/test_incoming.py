...
def test_get_brain_value_error(test_client):
    """Test that get_brain raises HTTP 400 on ValueError."""
    app.state.hivemind = Hivemind()  # initialize empty hivemind
    response = test_client.post("/start_trip?bike_id=999", json={"user_id": 1, "trip_id": 1})
    assert response.status_code == 400
    assert "does not exist" in response.text

def test_no_hivemind_raises_500(test_client):
...
def test_root_endpoint_no_hivemind_needed(test_client):
...
def test_start_trip_success(test_client, mock_brain):
    ...
    response = test_client.post("/start_trip", json={"user_id": 111, "trip_id": 222})
...
@pytest.mark.usefixtures("override_get_brain")
def test_start_trip_already_unlocked_error(test_client, mock_brain):
    ...
    response = test_client.post("/start_trip", json={"user_id": 1, "trip_id": 999})
    assert response.status_code == 400
    assert "Bike unlocked!" in response.text

@pytest.mark.usefixtures("override_get_brain")
def test_start_trip_moving_or_charging_error(test_client, mock_brain):
    ...
    response = test_client.post("/start_trip", json={"user_id": 123, "trip_id": 456})
    assert response.status_code == 400
    assert "Bike is moving or charging" in response.text

@pytest.mark.usefixtures("override_get_brain")
def test_start_trip_unexpected_exception(test_client, mock_brain):
    ...
    resp = test_client.post("/start_trip", json={"user_id": 99, "trip_id": 100})
...
@pytest.mark.usefixtures("override_get_brain")
def test_move_success(test_client, mock_brain):
    """Test moving the bike successfully."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
    mock_report = {"r": "report"}
    mock_brain.bike.reports.last = MagicMock(return_value=mock_report)
    resp = test_client.post("/move", json={"position_or_linestring": [12.45, 23.45]})
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.usefixtures("override_get_brain")
def test_move_invalid_position_error(test_client, mock_brain):
    ...
    resp = test_client.post("/move", json={"position_or_linestring": [9999, 9999]})
    assert resp.status_code == 400
    assert "Bad coordinates!" in resp.text

@pytest.mark.usefixtures("override_get_brain")
def test_move_moving_or_charging(test_client, mock_brain):
    """Test that MovingOrChargingError is caught and returns HTTP 400."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=True)
    resp = test_client.post("/move", json={"position_or_linestring": [12, 34]})
    assert resp.status_code == 400
    assert "Bike is moving or charging" in resp.text

@pytest.mark.usefixtures("override_get_brain")
def test_move_unexpected_exception(test_client, mock_brain):
    """Test that an unexpected exception is caught and returns HTTP 500."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
...
@pytest.mark.usefixtures("override_get_brain")
def test_move_already_locked(test_client, mock_brain):
    ...
    response = test_client.post("/move", json={"position_or_linestring": [12.45, 23.45]})
    assert response.status_code == 400
...
@pytest.mark.usefixtures("override_get_brain")
def test_relocate_success(test_client, mock_brain):
    """Test relocating the bike successfully."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
    mock_report = {"report": "some-report"}
    mock_brain.bike.reports.last = MagicMock(return_value=mock_report)
    resp = test_client.post("/relocate", json={"position": [11, 22]})
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.usefixtures("override_get_brain")
def test_relocate_moving_or_charging(test_client, mock_brain):
    """Test that MovingOrChargingError is caught and returns HTTP 400."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=True)
    resp = test_client.post("/relocate", json={"position": [11, 22]})
    assert resp.status_code == 400
...
@pytest.mark.usefixtures("override_get_brain")
def test_relocate_already_locked_error(test_client, mock_brain):
    """Test that AlreadyLockedError is caught and returns HTTP 400."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
    mock_brain.bike.relocate = MagicMock(side_effect=AlreadyLockedError("Bike is locked!"))
    response = test_client.post("/relocate", json={"position": [10.0, 20.0]})
    assert response.status_code == 400
    assert "Bike is locked!" in response.text

@pytest.mark.usefixtures("override_get_brain")
def test_relocate_invalid_position_error(test_client, mock_brain):
    """Test that InvalidPositionError is caught and returns HTTP 400."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
    mock_brain.bike.relocate = MagicMock(side_effect=InvalidPositionError("Bad position!"))

    response = test_client.post("/relocate", json={"position": [9999, 9999]})
    assert response.status_code == 400
    assert "Bad position!" in response.text

@pytest.mark.usefixtures("override_get_brain")
def test_relocate_unexpected_exception(test_client, mock_brain):
    """Test that an unexpected exception is caught and returns HTTP 500."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
...
@pytest.mark.usefixtures("override_get_brain")
def test_end_trip_success(test_client, mock_brain):
    ...
    mock_brain.bike.logs.last = MagicMock(return_value=mock_log)
    mock_brain.bike.reports.last = MagicMock(return_value=mock_report)
    resp = test_client.post("/end_trip", json={"maintenance": False, "ignore_zone": True})
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.usefixtures("override_get_brain")
def test_end_trip_moving_or_charging(test_client, mock_brain):
    """Test that MovingOrChargingError is caught and returns HTTP 400."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=True)
    resp = test_client.post("/end_trip", json={"maintenance": False, "ignore_zone": True})
    assert resp.status_code == 400
...
@pytest.mark.usefixtures("override_get_brain")
def test_end_trip_already_locked_error(test_client, mock_brain):
    """Test that AlreadyLockedError is caught and returns HTTP 400."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
...
@pytest.mark.usefixtures("override_get_brain")
def test_end_trip_unexpected_exception(test_client, mock_brain):
    """Test that an unexpected exception is caught and returns HTTP 500."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
    mock_brain.bike.lock = MagicMock(side_effect=RuntimeError("Something unexpected."))
    response = test_client.post("/end_trip", json={"maintenance": False, "ignore_zone": True})
    assert response.status_code == 500
...
@pytest.mark.usefixtures("override_get_brain")
def test_check_success(test_client, mock_brain):
    """Test checking the bike successfully."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
    mock_report = {"r": "report"}
    mock_brain.bike.reports.last = MagicMock(return_value=mock_report)
    resp = test_client.post("/check", json={"maintenance": True})
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.usefixtures("override_get_brain")
def test_check_moving_or_charging_error(test_client, mock_brain):
    """Test that MovingOrChargingError is caught and returns HTTP 400."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=True)
    resp = test_client.post("/check", json={"maintenance": False})
    assert resp.status_code == 400
    assert "Bike is moving or charging" in resp.text

@pytest.mark.usefixtures("override_get_brain")
def test_check_unexpected_exception(test_client, mock_brain):
    """Test that an unexpected exception is caught and returns HTTP 500."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
    mock_brain.bike.check = MagicMock(side_effect=RuntimeError("Unexpected check error!"))
    response = test_client.post("/check", json={"maintenance": True})
    assert response.status_code == 500
...
@pytest.mark.usefixtures("override_get_brain")
def test_report_success(test_client, mock_brain):
    """Test creating a report successfully."""
    mock_report = {"rep": "some-report"}
    mock_brain.bike.reports.last = MagicMock(return_value=mock_report)
    resp = test_client.get("/report")
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.usefixtures("override_get_brain")
def test_report_unexpected_exception(test_client, mock_brain):
    """Test that an unexpected exception is caught and returns HTTP 500."""
...
@pytest.mark.usefixtures("override_get_brain")
def test_log_success(test_client, mock_brain):
    """Test fetching the last log successfully."""
    mock_log = {"log": "some-log"}
    mock_brain.bike.logs.last = MagicMock(return_value=mock_log)
    resp = test_client.get("/log")
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.usefixtures("override_get_brain")
def test_log_unexpected_exception(test_client, mock_brain):
    """Test that an unexpected exception is caught and returns HTTP 500."""
...
@pytest.mark.usefixtures("override_get_brain")
def test_update_moving_or_charging(test_client, mock_brain):
    """Test that MovingOrChargingError is caught and returns HTTP 400."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=True)
    resp = test_client.post("/update", json={})
    assert resp.status_code == 400
    assert "Bike is moving or charging" in resp.text

@pytest.mark.usefixtures("override_get_brain")
def test_update_success(test_client, mock_brain):
...
@pytest.mark.usefixtures("override_get_brain")
def test_update_unexpected_exception(test_client, mock_brain):
    """Test that an unexpected exception is caught and returns HTTP 500."""
    mock_brain.bike.is_moving_or_charging = MagicMock(return_value=False)
    mock_brain.request_zones = AsyncMock(side_effect=RuntimeError("some unexpected error"))
    mock_brain.request_zone_types = AsyncMock(return_value={"zone_types": "data"})
    mock_brain.bike.update = MagicMock()
    response = test_client.post("/update", json={})
    assert response.status_code == 500
